...
nrow(mpg)
```
First method using str() gives output: tibble [# of rows x # of columns]
Second method using nrow/ncolumn gives direct answer
...
mpg$class <- gsub("compact","1", mpg$class)
mpg$class <- gsub("midsize","2", mpg$class)
mpg$class <- gsub("suv","3", mpg$class)
mpg$class <- gsub("2seater","4", mpg$class)
mpg$class <- gsub("pickup","5", mpg$class)
mpg$class <- gsub("subcompact","6", mpg$class)
mpg$class <- gsub("minivan","7", mpg$class)
mpg$class <- gsub("sub1","8", mpg$class)


mpg$class_numeric = sapply(mpg$class, as.integer, USE.NAMES = FALSE)
...
sub <- subset(mpg, class == "subcompact" & cty == 15,
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, shape = class)) + scale_shape_manual(values = c(0, 5, 10, 15, 21, 11, 4))
```
...
```{r}
#  (method 1 )
sizeMass$size_log10 <- log10(x)
sizeMass$mass_log10 <- log10(y)
m1 <- ggplot(sizeMass, aes(size_log10, mass_log10)) +
  geom_point() +
  geom_smooth(method = "lm", color = "red", se = FALSE)
...
plotxy <- ggplot(sizeMass, aes(size, mass)) +
